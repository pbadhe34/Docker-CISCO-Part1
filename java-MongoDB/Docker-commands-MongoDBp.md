  ...
  To Start a MongoDB container without volume

   docker run --name api-mongo  -d mongo

 Run MongoDB with voloume
  docker run --name api-mongo -v /mongo-data:/data/db -d mongo

 To link the mongo-Expresss view
  docker run -d --link api-mongo:mongo --name mongo-express -p 8081:8081 mongo-express

  Build the java app
  docker build -f Dockerfile-JavaMongo.txt -t api-docker-app .

    docker run -d api-docker-app . 

      
  Run java app with MongoDB Linking

   docker run --link api-mongo:api-database api-docker-app   
...
------------------------------------
 ...
 Find the specific op addresss of container

   docker inspect --format '{{ .NetworkSettings.IPAddress }}' tomcat-server
 

   To start the container
  docker start tomcat-server 

  docker start apache-srever 
...
docker run -p 8090:80 --name apache-server httpd:2.4
 ...
 To run an image in intercative(attached) mode
  docker run -it ubuntu bash
   ...
   docker run -p 9090:8080 --name tomcat-server -h myhost tomcat:8.0-jre8
   docker run -p 9080:80 --name apache-server httpd:2.4

   Run in detached mode   
     docker run -d -p 9090:8080 --name tomcat-server tomcat:8.0-jre8
  
   To connect to the container., start the Shell
...
docker images py-app:v1 --format "{{.ID}}: {{.Repository}}"
...
This is how piping a file into a container could be done for a build. The container’s ID will be printed after the build is done and the build logs could be retrieved using docker logs. This is useful if you need to pipe a file or something else into a container and retrieve the container’s ID once the container has finished running.
 ...
 Run the PostGres Database server

 docker run --name app-pg -e POSTGRES_PASSWORD=MySekret -e POSTGRES_USER=raju -e POSTGRES_DB=pg -d postgres
...
-------------------------------------
 ...
 Run the PG container with image
  ...
  docker run -d --name pg-database pg-server

----------------------------------------
  Connect to PostGres databse server

  docker exec -it  app-pg /bin/bash
  ...
  To view information about the current database 
   connection, type \conninfo.
  To list the database's tables and their
    respective owners, type \dt.
  To list all of the tables, views, and sequences
   in the database, type \z.
  ...
  create new records
    INSERT INTO pg_users (id,name,address) VALUES (1,'Baba','pune');

  INSERT INTO pg_users (id,name,address) VALUES (2,'Tuaua','Mumbai');

  Insert new record
 INSERT INTO pg_data (count,address,currenttime) VALUES (1,'192.56.55.66',234);

  INSERT INTO pg_data (count,address,currenttime) VALUES (4,'192.56.55.66',20);

INSERT INTO pg_data (count,address,currenttime) VALUES (12,'192.56.55.66',300);

   Verify by select query
  select * from pg_data;

  SELECT count FROM pg_data WHERE currenttime >= (SELECT max(currenttime) FROM pg_data) LIMIT 1

  SELECT count FROM pg_data WHERE currenttime <= (SELECT max(currenttime) FROM pg_data)

  SELECT count FROM pg_data WHERE currenttime <= (SELECT max(currenttime) FROM pg_data) LIMIT 1

  Run the pg-server container with given name

  docker run -d --name pg-database --hostname pg-host pg-server 

  docker run -d --name pg-database -h pg-host pg-server 

  Run the py-app container by linking the pg-database  container.

  docker run -it -d  --link pg-database:dbserver --name py-server -h py-host py-app 

   Run Without link to database

   docker run -it -d --name py-server -h py-host py-app 
